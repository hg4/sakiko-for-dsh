...
- **所有权靠令牌文件**（`%DSH_HOME%\sakiko\run\voice-watchdog.json`，原子写）：里面是本代的 `token`
  与插件**自己拉起**的服务清单 `{key,pid,port,createdMs,createdSrc}`（api 成功后再写一次、桥成功后再写一次）。
  `createdMs` 是该服务进程的**创建时间锚点**（host 用"亲眼看到 spawn 返回的时刻"记下，与真实
  `Win32_Process.CreationDate` 通常只差几 ms~几十 ms）——看门狗用它做 **PID 复用闸门**：
  现进程创建时间与它相差 >2s 就判"这个 pid 已是别人的进程"，fail-safe 不动手；旧世代令牌没有这个
  字段时退化为「令牌文件 mtime + 1500ms」。看门狗触发回收时**重新读**这个文件：**`token` 不匹配、
  或文件不在，就什么都不杀**（宁可漏杀，绝不误杀新一代/外来服务）。正常停止或卸载时插件先把该文件
  写成 `{"token":null}`，于是任何残留看门狗醒来也会立刻收工。
- **回收对象不是登记的那个 pid，而是它的整棵后代树**。原因（2026-09-13 在**真实 DSH** 上实测）：
  ...
  ② 若登记 pid 还活着（另一条路径），先过 PID 复用闸门，再 `taskkill /PID <pid> /T /F`；
  ③ 按端口兜底：`netstat -ano` 找持有者，**只有落在「已通过校验的目标集」里**才杀；
  被判为"pid 已被复用"的 pid 是**永久排除集**，端口兜底也不许把它认领回来（否则"拒绝回收"和
  "树杀成功"会在同一秒内自相矛盾地把无关进程整树杀掉）；其余只记一行日志放过；
  ④ **反复复验"端口空闲 ∧ 目标后代全部消失"，直到成功或 15s 超时** —— 只发一次 kill 不算完成，
  而且只验端口也不够（不监听端口的孙进程同样要清掉）。
- **进程查询不可用时（PowerShell/WMI 拿不到）不许盲杀**：降级模式只杀"能被端口持有者交叉证实"的
  登记 pid（该 pid 此刻确实在监听它登记的那个端口），证实不了就 fail-safe 跳过并记日志。
- **日志**与语音自启动同一份文件、**append 语义**：`%DSH_HOME%\sakiko\logs\voice-autostart.log`，
  看门狗的行带 `[watchdog]` 标记。该文件超过 1 MB 时由**插件进程**轮转到带时间戳的
  `.1-<ts>`（最多留 2 份；**看门狗进程永不轮转、只 append**）。旧写法是"先 `rm .1` 再 `rename`"，
  热重载时两代会互相删档，已修。
...
| `fallback-port`：登记启动器已死 **且** 关掉 PPID 链枚举（诊断开关 `--no-descendants`） | **契约在 2026-09-13 变了**：`allowed` 不再含"未经校验的登记 pid" ⇒ 这条诊断路径下端口持有者**认不出** ⇒ 日志记「不在本代**已校验**的目标集内 → 不做任何操作（fail-safe）」、**不杀**（方向按"宁可漏杀"选；生产路径永远带后代枚举，真实形状由 `positive` 覆盖） |
...
| `log-append`：两代 DSH 共用同一 `DSH_HOME` | 上一代的行在第二代起来之后仍可查；日志超过 1MB 轮转出**带时间戳**的 `.1-<ts>`（最多 2 份） |

**误杀类阻塞项的回归用例（2026-09-13 独立审查 ①②③④ + Minor）** —— 每条都做过"未修版本必须 FAIL /
修好后 PASS"的前后对照：

| 用例 | 钉住的行为 |
| --- | --- |
| `gate-reuse-legacy-nodesc` / `gate-reuse-legacy-desc`（① 审查 S6b） | 令牌匹配、登记 pid=X、**X 的现进程比令牌新 6000ms**（= PID 复用特征）**且 X 正监听登记端口** ⇒ X **不许被杀**；日志**不许**再出现「拒绝回收…已被复用」与「端口…确认属本代服务→树杀成功」这两行自相矛盾的话 |
| `gate-reuse-control` | 同构造但 X **不监听**端口（对照组）：本来就不会被杀 ⇒ 区分出"凶手就是端口兜底" |
| `degraded-not-listening`（②） | 进程表不可用时，**不监听**登记端口的登记 pid **不许被盲杀**，且日志写明"已降级、只杀端口可证实的" |
| `degraded-listening`（②） | 确实在监听登记端口的登记 pid **必须被杀**（降级下的正向路径） |
| `createdms-mismatch`（③） | 令牌 createdMs 与现进程相差 30000ms（>2s）⇒ 判 PID 复用，**不许杀** |
| `createdms-match`（③） | 相差 800ms（≤2s）⇒ 按正常路径处理，**杀掉** |
| `createdms-token`（③ host 侧） | 令牌里 services[0] 的 createdMs 与测试侧实测的真实 CreationDate **相差 ≤2s**（身份判据成立），且 createdSrc 只能是 `spawn` |
| `stop-port-recheck`（④） | `terminate` 只终结启动器、worker 仍占端口时：**不收摊看门狗**、令牌不置失效、如实报「未能停掉（端口仍在监听）」 |
| `watchdog-cwd`（Minor） | `SAKIKO_ROOT` 指向不存在的目录时看门狗仍必须起得来（spawn 的 cwd 用 MODULE_DIR） |

复跑：`node tools/test-watchdog.mjs`（加 `--plugin <另一份插件副本>` 可对比未修版本；只用临时端口
21880/21000，被占会自动换随机端口）。收尾会把"登记过的 pid"里**已被系统复用**的（命令行不是本测试的
进程）单独标出来，不误报残留。
