...
//   随之报红的有一批（独立复审实测 6 条：§4 组 4 条 + §9 上限断言 1 条，**按文件顺序最先**红的是 §4 的
//   `桥不可达 → 真的轮询了`），**其中包括** §9 的上限断言（遮蔽后首次探针就把 1.2s 预算推进虚拟时钟
//   ⇒ waitedMs=1200 同时超过 700/300 两个上限），而**不是**「虚拟时间 = 睡眠之和」那条等式
//   —— 实测遮蔽时它**仍然 PASS**（右边测到的也是同一次偏移，两边一起被污染）。
//
