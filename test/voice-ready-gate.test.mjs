...
//   预算照常真实计时，而且探针耗时**不**推进虚拟时钟。若这个前提不成立，**报红的不是**「虚拟时间 =
//   睡眠之和」那条等式 —— 独立复审按两种破坏方式各实测一次（2026-09-16，脚本在仓库外）：
//     · **字面遮蔽**（把 realHttpProbe 的 setTimeout 换成块内替身：预算进虚拟时钟，abort 回调改由
//       `realSetTimeout(fn, 0)` 立刻触发）⇒ 探针一探就 abort ⇒ 假桥自检连试 10 个端口都不通 ⇒
//       测试在 **§2 的 `listen()`** 处以 `[FATAL] 起不出可达的假桥` 退出（exit=2；实测 `[PASS]=11`、
//       `[FAIL]=0`、**没有汇总行**）—— §9 那两条 C-5 断言**根本没跑到**。
//     · **只把探针预算计进虚拟时钟**（探针本身仍可用）⇒ 实测红 6 条：§4 1 条（文件顺序最先红的是
//       `桥不可达 → 真的轮询了`，probes=1）+ §5 3 条 + §9 2 条（`voiceStability=false 确实更早结算`、
//       `两种情形都没有超出各自上限` ← waitedMs=1200 > 700/300）；而那条等式**仍然 PASS**。
//   ⇒ 两种破坏下它都不是守卫（一个 PASS、一个跑不到）。等式守的是**测量前提**，生产属性由 §9 的
//   「不超各自上限」守（正对照：把 host.mjs 的 `Math.min(INTERVAL, remaining)` 改成睡满 INTERVAL
//   ⇒ 实测报红，默认 720>700 / fallback 360>300）。
//
...
async function listen(port, body) {
  for (let attempt = 1; attempt <= 10; attempt++) {
    const s = await new Promise((resolve, reject) => {
      ...
      // 门只在真·bind 失败（EADDRINUSE 之类）时 reject。⚠️ 这条路径**到不了**：调用方一律传端口 0，
      // 由内核挑一个空闲端口（实测 listen(0) 的 address().port 必为 6679 这种非 0 值）⇒ 不会被占。
      // 换言之这里**没有**"换端口重试"这回事 —— 换端口靠的是下面那圈循环：探不通 ⇒ close ⇒ 再 listen(0)。
...
console.log('== 9) C-2 / C-5：上限取值与「不超上限」（独立复审 I-1：这两条原先零覆盖）==')
{
  ...
  // 前提自检：虚拟时间必须**恰好**等于块内睡眠之和 —— 即虚拟时钟的唯一推进源就是循环自己的睡眠
  // （探针耗时、外部定时器一律没掺进虚拟时间；探针用的是模块作用域的真定时器）。
  // ⚠️ 这条**抓得到什么、抓不到什么**（结论来自实测，不是推理）：
  //   · 抓得到「harness 漂移」：把旧的 `driveClock`（另起一个真 setInterval 推进虚拟时钟）加回来时，
  //     clock.t 里就混进了不属于 sleeps 的那部分 ⇒ 等式两边对不上、**这条报红**。
  //   · 抓不到「探针也被遮蔽」：实测把探针的定时器一起换成块内替身时，探针的 1.2s 预算既进 clock.t
  //     也进 sleeps，两边**一起**被污染 ⇒ 这条仍然 PASS。所以那种破坏由**上面那条上限断言**先报红。
  //   · 对**生产代码**它是恒真式：`waitedMs` 与 `sleeps` 都由同一个 setTimeout 替身喂出来，
  //     删掉生产里的 `Math.min` 也照样相等 —— 它守的是**测量前提**，不是生产属性。
  //     生产属性由上面「不超各自上限」那条守（正对照：把 `Math.min` 那行改成睡满 INTERVAL ⇒ 实测报红）。
  //   （原文写的是「或让探针也被遮蔽，这条会先报红」—— 复审实测证明那半句是错的，故改成上面的事实。）
