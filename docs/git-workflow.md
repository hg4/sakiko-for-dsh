...
5. **⚠️ 本机（Windows PowerShell 5.1）下，凡是带引号 / $ / 中文 / 行号统计的命令，不要走 PowerShell 内联**
   ...
   - **无 BOM 的 UTF-8 `.ps1` 会被按「系统 ANSI 代码页」读**（本机实测 ACP = **936 / gb2312**，不是 1252）
     ⇒ 脚本里的非 ASCII 字面量**静默变乱码**（实测：内容只有 `Write-Output "CJK:中文"` 的无 BOM 脚本，
     退出码仍是 0，但字面量被解析成 `CJK:` + `涓`(U+6D93) + 一个**私用区字符**(U+E15F) + `枃`(U+6783)
     —— 中间那个不显示，看着就像 `涓枃`；同一个文件加 UTF-8 BOM 后输出就是 `CJK:中文`）。
   ...
   - ⇒ **已有克隆升级到本条 `.gitattributes` 之后，工作树不会自动变成 LF**（实测：用一个全新克隆
     `git checkout` 本分支后，除**分支上内容改过的那几个文本文件**外，其余文本文件仍是 `i/lf w/crlf`
     —— 在 `699909c` 上实测 **75** 个、在 `ea44109` 上 **73** 个，个数 = 78 − 分支改过的文本文件数
     （⇒ 以本 tip 跑出来的真实个数为准：它会随分支继续改动而变）；
     而 `git status` 是**干净**的，所以你不会察觉）。要一次性刷成 LF（**先确认 `git status` 为空**）：
     ```powershell
     # ⚠️ 必须拼成**绝对路径**：$f 里是仓库相对路径，而 Remove-Item 按**当前目录**解析
     #    （实测从别处跑：会去删当前目录里的同名文件，仓库里一个都没刷新）
     $f = git -C $repo ls-files --eol | Select-String 'w/crlf' | ForEach-Object { $p = ($_.Line -split "`t")[-1]; Join-Path $repo $p }
